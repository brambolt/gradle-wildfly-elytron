...
class ElytronPluginSpec extends Specification {
  ...
  def setup() {
    buildFiles = [
      applyFalse:
        ElytronPluginFixture.applyFalse(testProjectDir),
      applyOnly:
        ElytronPluginFixture.applyOnly(testProjectDir),
      executeTasks:
        ElytronPluginFixture.executeTasks(testProjectDir)
    ]
  }
  ...
  def 'can execute tasks'() {
    when:
    def result = runTask(testProjectDir.root,
      '-b', buildFiles.executeTasks.name as String,
      'all', '--info', '--stacktrace')
    then:
